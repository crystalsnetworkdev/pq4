...
{ // Frodo-KEM's key encapsulation
    ...
    frodo_unpack(B, temp+BYTES_SEED_A, CRYPTO_PUBLICKEYBYTES - BYTES_SEED_A);
...
{ // Frodo-KEM's key decapsulation
    ...
    frodo_unpack(Bp, ct, (PARAMS_LOGQ*PARAMS_N*PARAMS_NBAR)/8);
    frodo_unpack(C, ct+(PARAMS_LOGQ*PARAMS_N*PARAMS_NBAR)/8, (PARAMS_LOGQ*PARAMS_NBAR*PARAMS_NBAR)/8);
    frodo_mul_bs(W, Bp, S);
    frodo_sub(W, C, W);
